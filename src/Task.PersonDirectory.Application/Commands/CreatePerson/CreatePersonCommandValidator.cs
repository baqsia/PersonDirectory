...
{
    public CreatePersonCommandValidator(IResourceLocalizer localizer)
    {
        ...
        RuleForEach(x => x.PhoneNumbers)
            .ChildRules(phone =>
            {
                phone.RuleFor(p => p.Type)
                    .IsInEnum()
                    .WithMessage(localizer.Localize(ResourceKeys.PhoneNumberTypeInvalid));

                phone.RuleFor(p => p.Number)
                    .NotEmpty()
                    .WithMessage(localizer.Localize(ResourceKeys.PhoneNumberIsRequired))
                    .Length(4, 50)
                    .WithMessage(localizer.Localize(ResourceKeys.PhoneNumberLengthRange));
            });
    }
}